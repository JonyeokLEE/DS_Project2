...
void BpTree::splitDataNode(BpTreeNode* pDataNode) {
	...
	if (pDataNode != root)
	{
		BpTreeNode* SplitParent = pDataNode->getParent();
	}
	...
	SplitParent->setMostLeftChild(pDataNode);
	pDataNode->setParent(SplitParent);
...
void BpTree::splitIndexNode(BpTreeNode* pIndexNode) {
	...
	if (pIndexNode != root)
	{
		BpTreeNode* SplitParent = pIndexNode->getParent();
	}
	...
	SplitParent->insertIndexMap(BeParent->first, qNode);
	SplitParent->setMostLeftChild(pIndexNode);
	pIndexNode->setParent(SplitParent);
	qNode->setParent(SplitParent);

	pIndexNode->getDataMap()->erase(BeParent->first);
	pIndexNode->getDataMap()->erase(q->first);
	pIndexNode->setMostLeftChild(pIndexNode->getMostLeftChild());
